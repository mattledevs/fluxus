 ...
 * @since fluxus 1.0
 */
function fluxus_html_classes() {

	global $fluxus_body_classes;

	if ( ! $fluxus_body_classes ) {
		return false;
	}

	if ( ! is_array($fluxus_body_classes) ) {
		return false;
	}

	$fluxus_body_classes = array_unique( $fluxus_body_classes );
	$fluxus_body_classes = array_map( 'esc_attr', $fluxus_body_classes );

	echo ' class="' . join( ' ', $fluxus_body_classes ) . '"';
...
function fluxus_add_html_class( $class ) {
	...
	if ( ! $fluxus_body_classes || ! is_array( $fluxus_body_classes ) ) {
		$fluxus_body_classes = array();
	}

	$fluxus_body_classes[] = $class;

	return $fluxus_body_classes;
